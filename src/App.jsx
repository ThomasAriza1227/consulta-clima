...
function App(){
 ...
 <input value={ciudad} placeholder="Ejemplo: Bogotá" onChange={e=>setCiudad(e.target.value)}/>
 <button onClick={buscarClima} disabled={cargando}>{cargando?"Consultando...":"Consultar clima"}</button>
 {error&&<p>{error}</p>}{lugar&&clima&&<article><h2>{lugar.name}</h2><p>País: {lugar.country}</p>
 <p>Temperatura: {clima.temperatura} °C</p><p>Sensación térmica: {clima.sensacion} °C</p><p>Viento: {clima.viento} km/h</p>
 <strong>{clima.estado}</strong></article>}</main>;
} export default App;